...
#[macro_export]
macro_rules! SECP256K1_GE_CONST {
    (
...
#[macro_export]
macro_rules! SECP256K1_GE_STORAGE_CONST_GET {
    ($t:expr) => {{
        let fe_x = SECP256K1_FE_STORAGE_CONST_GET!($t.x);
        let fe_y = SECP256K1_FE_STORAGE_CONST_GET!($t.y);
        (
            fe_x.0, fe_x.1, fe_x.2, fe_x.3, fe_x.4, fe_x.5, fe_x.6, fe_x.7,
            fe_y.0, fe_y.1, fe_y.2, fe_y.3, fe_y.4, fe_y.5, fe_y.6, fe_y.7
        )
    }};
}

#[macro_export]
macro_rules! SECP256K1_G_ORDER_13 {
    () => {
        SECP256K1_GE_CONST!(
...
#[macro_export]
macro_rules! SECP256K1_G_ORDER_199 {
    () => {
        SECP256K1_GE_CONST!(
...
pub fn secp256k1_ge_neg(r: &mut secp256k1_ge, a: &secp256k1_ge) {
    *r = a.clone();
    ...
    let y1 = r.y.clone();
    secp256k1_fe_negate(&mut r.y, &y1, 1);
}

pub fn secp256k1_ge_set_gej(r: &mut secp256k1_ge, a: &mut secp256k1_gej) {
    //secp256k1_fe z2, z3;
    let mut z2 = secp256k1_fe::new();
    let mut z3 = secp256k1_fe::new();
    r.infinity = a.infinity;
    let z1 = a.z.clone();
    secp256k1_fe_inv(&mut a.z, &z1);
    secp256k1_fe_sqr(&mut z2, &a.z);
    secp256k1_fe_mul(&mut z3, &a.z, &z2);
    let x1 = a.x.clone();
    secp256k1_fe_mul(&mut a.x, &x1, &z2);
    let y1 = a.y.clone();
    secp256k1_fe_mul(&mut a.y, &y1, &z3);
...
pub fn secp256k1_ge_set_gej_var(r: &mut secp256k1_ge, a: &mut secp256k1_gej) {
    let mut z2 = secp256k1_fe::new();
    let mut z3 = secp256k1_fe::new();
    if (a.infinity != 0) {
    ...
    let z1 = a.z.clone();
    secp256k1_fe_inv_var(&mut a.z, &z1);
    secp256k1_fe_sqr(&mut z2, &a.z);
    secp256k1_fe_mul(&mut z3, &a.z, &z2);
    let x1 = a.x.clone();
    secp256k1_fe_mul(&mut a.x, &x1, &z2);
...
fn secp256k1_ge_set_all_gej_var(r: &mut [secp256k1_ge], a: &[secp256k1_gej], len: usize) {
    ...
    for i in 0..len {
        if (a[i].infinity != 0) {
            ...
            if (last_i == usize::MAX) {
                ...
                let x_last = r[last_i].x.clone();
                secp256k1_fe_mul(&mut r[i].x, &x_last, &a[i].z);
            }
            last_i = i;
        }
    }
    ...
    while (i > 0) {
        i-=1;
        if (a[i].infinity == 0) {
            let r_clone = r[i].x.clone();
            secp256k1_fe_mul(&mut r[last_i].x, &r_clone, &u);
            let u1 = u.clone();
            secp256k1_fe_mul(&mut u, &u1, &a[last_i].z);
            last_i = i;
        }
    }
    ...
    for i in 0..len {
        if (a[i].infinity == 0) {
            let rx = r[i].x.clone();
            secp256k1_ge_set_gej_zinv(&mut r[i], &a[i], &rx);
        }
    }
}
...
pub fn secp256k1_ge_table_set_globalz(len: usize, a: &mut [secp256k1_ge], zr: &[secp256k1_fe]) {
    ...
    if (len > 0) {
        ...
        while (i > 0) {
            let mut tmpa = secp256k1_gej::new();
            if (i != len - 1) {
                let zs1 = zs.clone();
                secp256k1_fe_mul(&mut zs, &zs1, &zr[i]);
            }
            i-=1;
            tmpa.x = a[i].x.clone();
            tmpa.y = a[i].y.clone();
...
pub fn secp256k1_ge_set_xo_var(r: &mut secp256k1_ge, x: &secp256k1_fe, odd: i32) -> i32 {
    ...
    if (secp256k1_fe_sqrt(&mut r.y, &x3) == 0) {
        return 0;
    }
...
fn secp256k1_gej_eq_x_var(x: &secp256k1_fe, a: &secp256k1_gej) -> i32 {
    let mut r = secp256k1_fe::new();
    let mut r2 = secp256k1_fe::new();
    //VERIFY_CHECK(!a.infinity);
    secp256k1_fe_sqr(&mut r, &a.z);
    let r1 = r.clone();
    secp256k1_fe_mul(&mut r, &r1, x);
    r2 = a.x.clone();
...
pub fn secp256k1_ge_is_valid_var(a: &secp256k1_ge) -> i32 {
    ...
    secp256k1_fe_mul(&mut x3, &x3_clone, &a.x);
    secp256k1_fe_add(&mut x3, &secp256k1_fe_const_b);
...
fn secp256k1_gej_double(r: &mut secp256k1_gej, a: &secp256k1_gej) {
    ...
    let mut l = secp256k1_fe::new();
    let mut s = secp256k1_fe::new();
    let mut t = secp256k1_fe::new();
...
pub fn secp256k1_gej_add_var(r: &mut secp256k1_gej, a: &secp256k1_gej, b: &secp256k1_gej, rzr: Option<&mut secp256k1_fe>) {
    ...
    let mut z22= secp256k1_fe::new();
    let mut z12= secp256k1_fe::new();
    let mut u1= secp256k1_fe::new();
    let mut u2= secp256k1_fe::new();
    let mut s1= secp256k1_fe::new();
    let mut s2= secp256k1_fe::new();
    let mut h= secp256k1_fe::new();
    let mut i= secp256k1_fe::new();
    let mut h2= secp256k1_fe::new();
    let mut h3= secp256k1_fe::new();
    let mut t= secp256k1_fe::new();
    ...
    secp256k1_fe_mul(&mut s1, &a.y, &z22); 
    let s1_clone = s1.clone();
    secp256k1_fe_mul(&mut s1, &s1_clone, &b.z);
    secp256k1_fe_mul(&mut s2, &b.y, &z12);
    let s2_clone = s2.clone();
    secp256k1_fe_mul(&mut s2, &s2_clone, &a.z);
    ...
    secp256k1_fe_negate(&mut h2, &h2_clone, 1);
    ...
    secp256k1_fe_mul(&mut h3, &h3_clone, &s1);
...
pub fn secp256k1_gej_add_zinv_var(r: &mut secp256k1_gej, a: &secp256k1_gej, b: &secp256k1_ge, bzinv: &secp256k1_fe) {
    ...
    if a.infinity != 0{
        ...
        let mut bzinv2= secp256k1_fe::new();
        let mut bzinv3= secp256k1_fe::new();
    ...
    secp256k1_fe_mul(&mut s2, &b.y, &z12);
    let s2_clone = s2.clone();
    secp256k1_fe_mul(&mut s2, &s2_clone, &az);
    secp256k1_fe_negate(&mut h, &u1, 1); secp256k1_fe_add(&mut h, &u2);
    secp256k1_fe_negate(&mut i, &s2, 1); secp256k1_fe_add(&mut i, &s1);
    if (secp256k1_fe_normalizes_to_zero_var(&h) != 0) {
        if (secp256k1_fe_normalizes_to_zero_var(&i) != 0) {
...
pub fn secp256k1_gej_add_ge(r: &mut secp256k1_gej, a: &secp256k1_gej, b: &secp256k1_ge) {
    ...
    let mut zz= secp256k1_fe::new();
    let mut u1= secp256k1_fe::new();
    let mut u2= secp256k1_fe::new();
    let mut s1= secp256k1_fe::new();
    let mut s2= secp256k1_fe::new();
    let mut t= secp256k1_fe::new();
    let mut tt= secp256k1_fe::new();
    let mut m= secp256k1_fe::new();
    let mut n= secp256k1_fe::new();
    let mut q= secp256k1_fe::new();
    let mut rr= secp256k1_fe::new();
    let mut m_alt= secp256k1_fe::new();
    let mut rr_alt= secp256k1_fe::new();
    ...
    u1 = a.x.clone();
    secp256k1_fe_normalize_weak(&mut u1);        /* u1 = U1 = X1*Z2^2 (1) */
    secp256k1_fe_mul(&mut u2, &b.x, &zz);                  /* u2 = U2 = X2*Z1^2 (1) */
    s1 = a.y.clone();
    secp256k1_fe_normalize_weak(&mut s1);        /* s1 = S1 = Y1*Z2^3 (1) */
    secp256k1_fe_mul(&mut s2, &b.y, &zz);                  /* s2 = Y2*Z1^2 (1) */
    let s2_clone = s2.clone();
    secp256k1_fe_mul(&mut s2, &s2_clone, &a.z);                  /* s2 = S2 = Y2*Z1^3 (1) */
    t = u1.clone();
    secp256k1_fe_add(&mut t, &u2);                  /* t = T = U1+U2 (2) */
    m = s1.clone();
    secp256k1_fe_add(&mut m, &s2);                  /* m = M = S1+S2 (2) */
    ...
    let q_clone = q.clone();
    secp256k1_fe_mul(&mut q, &q_clone, &n);                       /* q = Q = -T*Malt^2 (1) */
    /* These two lines use the observation that either M == Malt or M == 0,
     * so M^3 * Malt is either Malt^4 (which is computed by squaring), or
     * zero (which is "computed" by cmov). So the cost is one squaring
     * versus two multiplications. */
     let n1 = n.clone();
    secp256k1_fe_sqr(&mut n, &n1);
    ...
    r.x = t.clone();                                           /* r.x = X3 = Ralt^2 + Q (2) */
    secp256k1_fe_mul_int(&mut t, 2);                        /* t = 2*X3 (4) */
    secp256k1_fe_add(&mut t, &q);                           /* t = 2*X3 + Q (5) */
    let t1 = t.clone();
    secp256k1_fe_mul(&mut t, &t1, &rr_alt);                  /* t = Ralt*(2*X3 + Q) (1) */
...
pub fn secp256k1_gej_rescale(r: &mut secp256k1_gej, s: &secp256k1_fe) {
    ...
    let x1 = r.x.clone();
    secp256k1_fe_mul(&mut r.x, &x1, &zz);                /* r.x *= s^2 */
    let y1 = r.y.clone();
    secp256k1_fe_mul(&mut r.y, &y1, &zz);
    let y1 = r.y.clone();
    secp256k1_fe_mul(&mut r.y, &y1, s);                  /* r.y *= s^3 */
    let z1 = r.z.clone();
    secp256k1_fe_mul(&mut r.z, &z1, s);                  /* r.z *= s   */
}
...
fn sec256k1_gej_cmov(r: &mut secp256k1_gej, a: &secp256k1_gej, flag: i32) {
    secp256k1_fe_cmov(&mut r.x, &a.x, flag);
    secp256k1_fe_cmov(&mut r.y, &a.y, flag);
    secp256k1_fe_cmov(&mut r.z, &a.z, flag);
...
pub fn secp256k1_ge_mul_lambda(r: &mut secp256k1_ge, a: &secp256k1_ge) {
    *r = a.clone();
    let x1 = r.x.clone();
    secp256k1_fe_mul(&mut r.x, &x1, &secp256k1_const_beta);
}
