...
//     r->d[3] = t & nonzero;
// }
fn secp256k1_scalar_negate(r: &mut secp256k1_scalar, a: &secp256k1_scalar) {
...
//     return 2 * (mask == 0) - 1;
// }
fn secp256k1_scalar_cond_negate(r: &mut secp256k1_scalar, flag: i32) {
    /* If we are flag = 0, mask = 00...00 and this is a no-op;
     * if we are flag = 1, mask = 11...11 and this is identical to secp256k1_scalar_negate */
    let mut mask = (!flag) - 1;
    let mut nonzero = (secp256k1_scalar_is_zero(r) != 0) as u64 - 1;
    let mut t = (r.d[0] ^ mask) + ((SECP256K1_N_0 + 1) & mask);
    r.d[0] = t & nonzero; t >>= 64;
    t += (r.d[1] ^ mask) + (SECP256K1_N_1 & mask);
    r.d[1] = t & nonzero; t >>= 64;
    t += (r.d[2] ^ mask) + (SECP256K1_N_2 & mask);
    r.d[2] = t & nonzero; t >>= 64;
    t += (r.d[3] ^ mask) + (SECP256K1_N_3 & mask);
    r.d[3] = t & nonzero;
    return 2 * (mask == 0) - 1;
...
fn secp256k1_scalar_reduce_512(r: &mut secp256k1_scalar, l: &[u64; 8]) {
    ...
    /* m[0..6] = l[0..3] + n[0..3] * SECP256K1_N_C. */
    c0 = l[0]; c1 = 0; c2 = 0;
    muladd_fast!(n0, SECP256K1_N_C_0, c0, c1, c2);
    ...
    /* p[0..4] = m[0..3] + m[4..6] * SECP256K1_N_C. */
    c0 = m0; c1 = 0; c2 = 0;
    muladd_fast!(m4, SECP256K1_N_C_0, c0, c1, c2);
    ...
    muladd_fast!(m6, SECP256K1_N_C_1, c0, c1, c2);
...
fn secp256k1_scalar_mul_512(l: &mut [u64; 8], a: &secp256k1_scalar, b: &secp256k1_scalar) {
    ...
    /* l[0..7] = a[0..3] * b[0..3]. */
    muladd_fast!(a.d[0], b.d[0], c0, c1, c2);
    ...
    muladd_fast!(a.d[3], b.d[3], c0, c1, c2);
...
// }
fn secp256k1_scalar_cmov(r: &mut secp256k1_scalar, a: &secp256k1_scalar, flag: i32) {
    let mut mask0: u64;
    let mut mask1: u64;
    VG_CHECK_VERIFY(r.d, std::mem::size_of_val(&r.d));
    mask0 = flag + !((0 as u64) as u64);
    mask1 = !mask0;
    r.d[0] = (r.d[0] & mask0) | (a.d[0] & mask1);
    r.d[1] = (r.d[1] & mask0) | (a.d[1] & mask1);
    r.d[2] = (r.d[2] & mask0) | (a.d[2] & mask1);
...
const secp256k1_const_modinfo_scalar: secp256k1_modinv64_modinfo = secp256k1_modinv64_modinfo {
    ...
    modulus_inv: 0x34F20099AA774EC1
};
