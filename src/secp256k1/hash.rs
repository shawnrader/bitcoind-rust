...
pub fn secp256k1_sha256_write(hash: &mut secp256k1_sha256, data: &[u8], len: usize) {
    let mut bufsize = hash.bytes & 0x3F;
    ...
    while len >= 64 - bufsize {
        ...
        hash.buf[bufsize..(bufsize + chunk_len)].copy_from_slice(&data[0..chunk_len]);
        //memcpy(hash->buf + bufsize, data, chunk_len);
        //data += chunk_len;
        //len -= chunk_len;
...
pub fn secp256k1_sha256_finalize(hash: &mut secp256k1_sha256, out32: &mut [u8; 32]) {
    ...
    secp256k1_write_be32(&mut sizedesc[0..4], (hash.bytes >> 29) as u32);
    secp256k1_write_be32(&mut sizedesc[4..8], (hash.bytes << 3) as u32);
    secp256k1_sha256_write(hash, &PAD[0..(1 + ((119 - (hash.bytes % 64)) % 64))], 1 + ((119 - (hash.bytes % 64)) % 64));
    secp256k1_sha256_write(hash, &sizedesc, 8);
    for i in 0..8 {
